...
namespace scp_294.Items.DrinkFeatures
{
    public class Effect
    {
        ...
        /// </summary>
        public Intensity İntensity { get; set; }
        ...
        public void ApplyEffect(Player player, bool addToIntensity = false)
        {
            if (Roll(Chance))
            {
                Log.Debug($"Applying effect to player. Duration: {Duration}. Intensity fixed amount: {İntensity.FixedAmount}. Intensity range: {İntensity.LowestAmount} to {İntensity.HighestAmount}");

                byte currInt = (byte)(addToIntensity ? player.GetEffect(EffectType.MovementBoost).Intensity : 0);

                player.ChangeEffectIntensity(Type, (byte)Mathf.Clamp(currInt + İntensity.GetIntensity(), 0, 255), Duration);
            }
        }
